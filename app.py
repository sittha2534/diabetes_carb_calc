...
if st.button("🔍 คำนวณผลลัพธ์"):
    ...
    for meal, carb_g_m in zip(meal_names, carb_split):
        ...
        # เมนูตัวอย่าง
        if meal == "เช้า":
            menu = f"ข้าวสวย {rice_cup} ทัพพี + ไข่ต้ม 1 ฟอง + ผักสด"
        elif meal == "กลางวัน":
            menu = f"ข้าว {rice_cup} ทัพพี + อกไก่ย่าง {prot_portion} ชิ้น + แกงจืดผัก"
        elif meal == "เย็น":
            menu = f"ข้าว {rice_cup} ทัพพี + ปลาเผา {prot_portion} ชิ้น + ผักต้ม"
        else:  # ของว่าง
            menu = f"ผลไม้ 1 ส่วน เช่น แอปเปิ้ล 1 ลูกเล็ก หรือ ฝรั่งครึ่งลูก"

        meal_data.append([
            meal,
            carb_g_m, f"{carb_portion} portion", f"{rice_cup} ทัพพี",
            protein_per_meal, f"{prot_portion} portion", menu
        ])

    df_meal = pd.DataFrame(meal_data, columns=[
        "มื้ออาหาร",
        "คาร์บ (g)", "portion คาร์บ", "≈ ข้าว (ทัพพี)",
        "โปรตีน (g)", "portion โปรตีน", "เมนูอาหารแนะนำ"
    ])
